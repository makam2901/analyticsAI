...
def _generate_response(prompt: str, model: str = "gemini-2.5-flash") -> str:
...
async def generate_code(request: CodeGenerationRequest):
    ...
    try:
        ...
        # Step 2: Generate data loading code for kernel
        data_loading_code = ["import pandas as pd", "from google.cloud import storage", "import tempfile", "import os"]
        
        for var_name, file_info in file_mapping.items():
            filename = file_info['filename']
            bucket_name = file_info['bucket']
            data_loading_code.append(f"""
# Load {filename} from {bucket_name}
...
blob.download_to_filename(local_path)
{var_name} = pd.read_csv(local_path)
print(f"Loaded {filename} as {var_name} with shape: {{{var_name}.shape}}")
""")
        
        # Step 3: Build context for LLM (dataframes are loaded in kernel)
        context_str = ""
        for var_name, file_info in file_mapping.items():
...
""")
            ...
            # Step 3: Combine data loading + user's aggregation code
            full_code = "\n".join(data_loading_code) + "\n" + request.code
            ...
            # For SQL, we'd need to implement SQL execution
            return CodeExecutionResponse(
                success=False,
